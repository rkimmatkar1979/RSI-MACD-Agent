...
def get_ai_recommendations(shortlist_df):
    ...
    prompt = (
        ...
        f"concise professional trading note. For EACH stock, write exactly 4 "
        "sentences covering: (1) WHY this is a buy/sell candidate right now - the "
        "confluence of signals (Fibonacci level, RSI, MACD, volume, sector trend) "
        "that makes this setup attractive, (2) WHEN to enter - e.g. immediately at "
        "CMP, on a pullback to a specific level, or on a breakout above a specific "
        "level, (3) Stop-Loss level with a brief rationale, and (4) Take-Profit "
        "target. Calibrate all levels for a SWING TRADE with a 2-3 WEEK holding "
        "horizon (not an intraday or scalp trade) - use the given Fibonacci range, "
        "52-week range, and current price action to justify the levels. If a news "
        "headline is material (e.g. earnings, regulatory action, M&A, guidance "
        "change), briefly factor it into the plan; ignore routine or irrelevant "
        "headlines. Use a clear heading for each ticker."
    )
