...
def search(feature_df, label_col="target", feature_cols=FEATURE_COLUMNS, n_iter=20, n_folds=3, random_state=42):
    """Returns a list of (mean_auc, params) sorted best-first."""
    df = feature_df.dropna(subset=[label_col]).sort_values("date").reset_index(drop=True)
    X, y = df[feature_cols], df[label_col]

    all_splits = list(purged_time_series_splits(df["date"]))
...
def main(period="5y", n_iter=20, n_folds=3, feature_set="broadened", label_kind="relative"):
    ...
    print(f"[tune] computing {label_kind} labels...")
    labels = (
        relative_triple_barrier_labels(price_df, index_df) if label_kind == "relative"
        else triple_barrier_labels(price_df)
    )
    merged = features.merge(labels, on=["ticker", "date"], how="left")

    print(f"[tune] searching {n_iter} candidates over the last {n_folds} purged folds...")
    results = search(merged, feature_cols=feature_cols, n_iter=n_iter, n_folds=n_folds)
