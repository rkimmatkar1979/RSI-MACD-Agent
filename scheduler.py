...
(scan -> score -> AI commentary -> persist) is triggered by one of:
  ...
  - should_auto_scan() below, checked once per app.py page load - a
    lightweight "whoever loads the page first each trading day triggers that
    day's scan, visibly, on their own screen" substitute for real
    scheduling (see app.py: a True result sets scan_in_progress and reruns
    into the exact same full-page progress view as the button above), or
...
# In-process guard for should_auto_scan() below: an in-memory dict (not the
# DB) because the DB check alone isn't enough - the triggering visitor's
# scan takes a couple of minutes to run, and every other visitor loading the
# page during that window would still see "no scan for today yet" and would
# each separately decide to trigger their own without this. Streamlit apps
# this size run as a single process, so a plain module-level dict is a
# valid process-wide lock; it resets (and a fresh auto-scan can fire again)
# if the process restarts, which is fine - worst case is one extra scan.
_auto_scan_state = {"triggered_date": None, "lock": threading.Lock()}


def should_auto_scan():
    """
    True the first time this is called on a trading day with no scan yet
    recorded for that date - a "no scheduling infra" substitute for a real
    daily cron: whichever visitor happens to load the page first each day
    triggers that day's scan for everyone, instead of requiring someone to
    remember to click "Run Full Scan Now".

    Deliberately does NOT run the scan itself or use a background thread -
    the caller (app.py) should respond to a True result by setting
    scan_in_progress=True and rerunning, so the triggering visitor sees the
    SAME full-page progress view as clicking the button themselves, not a
    scan running silently out of sight. Guarded by an in-process lock + date
    check so concurrent visitors during that visible scan's run time don't
    each also decide to trigger - only the first caller per day gets True.
    """
    ...
    return today not in db_handler.get_available_scan_dates()
